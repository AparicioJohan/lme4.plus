...
lme4.plot <-
  function(x,Datos,col="col",row="row",gen="line",all.in.one = TRUE, main = NULL, annotated = FALSE, depict.missing = FALSE, ...) {
    ...
    df <- data.table(columns = x.coord, rows = y.coord, 
                     response = response, fitted = fitted,
    ...
    main.legends <- c('Raw data', 'Fitted data',  "Ind Residuals" , 'Residuals', "Genotypic BLUPs", 'Histogram')
    if(all.in.one) {
    ...
    if(!all.in.one)
      readline("Press return for next page....")
    fields::image.plot(columns, rows, t(matrix(df$residuals, ncol = length(columns), nrow = length(rows))), main = main.legends[4], col = colors, xlab = xlab, ylab = ylab, graphics.reset = TRUE, ...)
    if(!all.in.one)
      readline("Press return for next page....")
